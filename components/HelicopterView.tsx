...
import { UserStory, Role, SprintSquad, MandaysByRole } from '../types';
...
export const HelicopterView: React.FC<{ store: SprintStore }> = ({ store }) => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-50 overflow-hidden" onClick={() => setSelectedTraceStoryId(null)}>
      ...
      <div className="flex-1 overflow-auto relative p-6 pt-0 custom-scrollbar">
        <div className="inline-grid gap-4" style={{ gridTemplateColumns: `minmax(200px, 1fr) repeat(${sprints.length}, minmax(320px, 1fr))` }}>
            {/* Header Row (Sticky) */}
            <div className="sticky top-0 z-20 bg-slate-50 py-4 font-black text-slate-400 uppercase text-[10px] tracking-widest self-end">Squads / Timeline</div>
            {sprints.map(sprint => (
            <div key={sprint.id} className="sticky top-0 z-20 bg-white border-b-4 border-slate-300 p-4 rounded-t-lg shadow-sm">
            ...
            <React.Fragment key={squad.id}>
                <div className="flex flex-col justify-center bg-white p-4 rounded-lg shadow-sm border border-slate-100 group relative sticky left-0 z-10">
                <span className="text-[10px] font-black text-indigo-500 uppercase mb-1 tracking-widest leading-none">Squad</span>
                <span className="text-lg font-black text-slate-800 tracking-tight">{squad.name}</span>
                </div>
                ...
                if (!sprintSquad) {
                    ...
                    <div 
                        key={sprint.id} 
                        className="group border-2 border-dashed border-slate-200 rounded-lg flex items-center justify-center bg-slate-100/30 hover:bg-indigo-50 hover:border-indigo-200 cursor-pointer transition-all h-48"
                        onClick={() => store.addSprintSquad(sprint.id, squad.id)}
                    >
                        <span className="text-slate-400 group-hover:text-indigo-400 text-xs font-black uppercase tracking-widest">+ Assign Squad</span>
                    </div>
                    );
                }
                ...
                return (
                    ...
                    className={`relative p-4 rounded-xl border-2 transition-all group min-h-[192px] ${status.bg} ${status.border} ${isOverloaded ? 'animate-pulse-subtle' : ''}`}
                    >
                    ...
                    <div className="space-y-1.5 max-h-40 overflow-y-auto no-scrollbar pb-2">
                        ...
                        return (
                            ...
                            className={`text-[11px] font-bold py-1.5 px-2 bg-white rounded shadow-sm flex items-center justify-between border-2 transition-all cursor-pointer active:cursor-grabbing ${isTraceRelated ? 'border-indigo-600 bg-indigo-50 ring-2 ring-indigo-200' : hasViolation ? 'border-red-400 bg-red-50' : 'border-slate-100 group-hover:border-slate-200'}`}
                            draggable
